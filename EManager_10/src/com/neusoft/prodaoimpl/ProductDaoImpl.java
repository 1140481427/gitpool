...
public class ProductDaoImpl implements ProductDao {
	...
	public boolean addPro(T_Product_List pro) throws DaoException {
		...
		try {
			int count = qr.update(conn, sql, pro.getPro_item_no(),pro.getPro_name(),pro.getPro_category_no(),pro.getPro_spec_no(),pro.getPro_img_src(),pro.getPro_price(),pro.getPro_hits(),pro.getIsRecommand(),pro.getIsSpecial_offer(),pro.getIsUp_Line(),pro.getPro_paritculars());
			if(count > 0){
				return true; 
			}
		} catch (SQLException e) {
	...
	public boolean updatePro(T_Product_List pro) throws DaoException {
		...
		try {
			int count = qr.update(conn, sql, pro.getPro_item_no(),pro.getPro_name(),pro.getPro_category_no(),pro.getPro_spec_no(),pro.getPro_img_src(),pro.getPro_price(),pro.getPro_hits(),pro.getIsRecommand(),pro.getIsSpecial_offer(),pro.getIsUp_Line(),pro.getPro_paritculars(),pro.getPid());
			if(count > 0){
				return true; 
			}
		} catch (SQLException e) {
	...
	public PageModel<T_Product_List> showPro(int pageNo, int pageSize) throws DaoException {
		...
		try {
			...
			sbSql.append("select * from ")
				...
				.append("from ( ")
				.append("select * from T_Product_List order by pid desc ")
	...
	@Override
	public PageModel<T_Product_List> showProByHot(int pageNo, int pageSize, String name) throws DaoException {
		PageModel<T_Product_List> pageModel = new PageModel<T_Product_List>();
		List<T_Product_List> list = new ArrayList<T_Product_List>();
		int num = 0;
		try {
			String sqlcount = "select count(pid) from t_product_list where  pro_name like '%" + name + "%'" ;
			Object obj = qr.query(conn, sqlcount, new ScalarHandler<Object>());
			num = Integer.parseInt(obj.toString());
			StringBuffer sbSql = new StringBuffer();
			sbSql.append("select * from ")
				.append("( ")
				.append("select rownum as rn, t.* ")
				.append("from ( ")
				.append("select * from t_product_list where  pro_name like '%" + name + "%'" )
				.append(") t where rownum <= ? ")
				.append(") where rn>? ");
			String sql = sbSql.toString();
			list = qr.query(conn, sql, new BeanListHandler<T_Product_List>(T_Product_List.class),pageNo * pageSize,(pageNo - 1) * pageSize);
			...
			throw new DaoException("根据名字分页查询失败!",e);
		}
		return pageModel;
	}
