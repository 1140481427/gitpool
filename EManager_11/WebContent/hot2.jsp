...
<html>
	...
	<script type="text/javascript">
		function topPage() {
			window.self.location = "showonline?pageNo=${pm.topPageNo}";
			
		}
		
		function previousPage() {
			window.self.location = "showonline?pageNo=${pm.previousPageNo}";
		}
		
		function nextPage() {
			window.self.location = "showonline?pageNo=${pm.nextPageNo}";
		}
	...
	<div class="container-fluid">
		...
		<div style="margin-left:40%" >
			<nav aria-label="Page navigation">
				    ...
				    <c:forEach begin="1" end="${pm.totalPageNo }" step="1" var="i">
				    	<c:choose>
				    		<c:when test="${pm.pageNo == i }">
				    			<li class="active"><a href="showonline?pageNo=${i }">${i }</a></li>
				    		</c:when>
				    		<c:otherwise>
				    			<li><a href="showonline?pageNo=${i }">${i }</a></li>
				    		</c:otherwise>
				    	</c:choose>
			  		</c:forEach>
				...
				&nbsp;第<select onChange="if(!isNaN(this.value)){window.self.location = 'showonline?pageNo=' + this.value }">
